...
export default function DailyIncomeExpenseGrid({ rawData = [], onUpdateData }) {
  ...
  // Phần này chỉ sắp xếp dữ liệu mới nhất lên đầu.
    ...
    <div className="common-grid-container">
      {gridData.length === 0 ? (
        ...
        gridData.map((item, index) => (
          <div key={index} className="grid-card" onClick={() => handleOpenPopup(item.originalItem)} style={{ cursor: "pointer" }}>
            ...
            <div className="grid-body">
              ...
              >
                {Object.entries(item.incomeDetails || {}).map(([key, val]) => {
                  if (key === "Ghi chú") {
                    return <React.Fragment key={key}>{renderNoteContent(val, "#28a745")}</React.Fragment>;
                  }
                ...
                {Object.entries(item.expenseDetails || {}).map(([key, val]) => {
                  if (key === "Ghi chú") {
                    return <React.Fragment key={key}>{renderNoteContent(val, "#d9534f")}</React.Fragment>;
                  }
              ...
              >
                ...
                <span
                  className={`value total ${item.dailyNet < 0 ? "text-danger" : "text-success"}`}
                  style={{
                    color: item.dailyNet < 0 ? "#d9534f" : "#28a745",
                  }}
                >
